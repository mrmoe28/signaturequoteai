...
## PDF Generation Error: 500 Internal Server Error on Vercel

### Symptom
- PDF generation endpoint `/api/quotes/[id]/pdf` returns 500 error
- Error message: "Failed to load resource: the server responded with a status of 500"
- Works locally but fails in Vercel serverless environment

### Root Cause
- Puppeteer cannot run in serverless environments without special configuration
- Vercel functions need `@sparticuz/chromium` package instead of regular Chromium
- Default Puppeteer configuration doesn't detect serverless environment
- Insufficient memory and timeout for PDF generation function

### Fix
1. Update `lib/pdf-generator.tsx` to use `@sparticuz/chromium` in serverless environments:
   ```typescript
   ...
   browser = await puppeteer.launch({
     args: isServerless ? chromium.args : [...standardArgs],
...
2. Update `vercel.json` to provide sufficient resources:
   ```json
...
3. Ensure build configuration skips Puppeteer's bundled Chromium:
   ```json
   "build": {
     "env": {
...
### Verification Checklist
- PDF generation works locally in development
