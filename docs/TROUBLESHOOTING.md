...
## Vercel Build Error: ERESOLVE due to nodemailer v7 with next-auth 5 beta

### Symptom

- Vercel deployment fails during `npm install` with `npm error ERESOLVE could not resolve` and messages like:

  ```
  While resolving: next-auth@5.0.0-beta.29
  Found: nodemailer@7.x
  Could not resolve dependency: peerOptional nodemailer@^6.6.5 from next-auth@5.0.0-beta.29
  ```

### Root Cause

- `next-auth@5.0.0-beta.29` and `@auth/core@0.40.0` have a peerOptional requirement on `nodemailer@^6`.
- Project was using `nodemailer@7`, which violates the peer range and causes npm to fail resolution on Vercel.

### Fix

1. Pin `nodemailer` to v6:
...
2. Remove `@types/nodemailer` dependency if present (types are bundled with nodemailer v6+ and not needed).

3. Reinstall dependencies locally to update the lockfile and verify:

   ```bash
   npm install
   ```

4. Commit `package.json` and `package-lock.json`, then redeploy on Vercel.

### Alternatives

- Upgrade `next-auth` to a version that supports `nodemailer@7` (if/when available) and adjust related `@auth/*` packages accordingly.
- As a temporary workaround only, `npm install --legacy-peer-deps` can bypass peer checks, but it is not recommended for CI/Vercel stability.

### Verification Checklist

- `npm install` succeeds locally and on Vercel.
- Build proceeds past dependency installation.
