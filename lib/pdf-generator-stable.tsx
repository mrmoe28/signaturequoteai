...
export function generateSimpleTextPDF(quote: Quote): Buffer {
  const content = `
QUOTE #${quote.id}
Date: ${quote.createdAt ? new Date(quote.createdAt).toLocaleDateString() : 'N/A'}
...
${quote.items.map(item => 
  `${item.name} - Qty: ${item.quantity} x $${(item.unitPrice || 0).toFixed(2)} = $${item.extended.toFixed(2)}`
).join('\n')}

Subtotal: $${quote.items.reduce((sum, item) => sum + item.extended, 0).toFixed(2)}
Tax (8%): $${(quote.items.reduce((sum, item) => sum + item.extended, 0) * 0.08).toFixed(2)}
Total: $${(quote.items.reduce((sum, item) => sum + item.extended, 0) * 1.08).toFixed(2)}
