...
// Initialize Gmail API with proper configuration based on Google OAuth 2.0 documentation
function getGmailService() {
  ...
  // This follows Google's OAuth 2.0 service account documentation
...
// Enhanced connectivity check with proper error handling
...
function createEmailMessage({ to, subject, html, text, pdfBuffer, quoteNumber }: {
  ...
  let message = [
    `From: ${process.env.BUSINESS_EMAIL || process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL}`,
    `To: ${to}`,
    `Subject: ${subject}`,
    'MIME-Version: 1.0',
